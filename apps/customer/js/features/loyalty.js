...
export function renderLoyalty(){
  ...
  document.getElementById('page-loyalty').innerHTML=`<div class="wrap section">
    <div class="loy-card"><div class="loy-card-mesh"></div><div class="loy-tier">${icon('star',{size:15,fill:true})} سطح طلایی</div><div class="loy-pts"><span id="loyPts" class="loy-pts-num">${fmtFa(pts)}</span> <span style="font-size:20px;font-weight:600;opacity:.9">امتیاز</span></div><div class="loy-next">۱۶۰ امتیاز تا سطح بعدی (پلاتینیوم)</div><div class="loy-prog"><div class="loy-prog-fill" style="width:0" data-w="68"></div></div></div>
    ...
    <div class="badge-grid reveal">${BADGES.map(b=>`<div class="bdg ${b[2]?'earned':'locked'}"><div class="bdg-emoji">${b[0]}</div><div class="bdg-name">${b[1]}</div><div class="bdg-desc">${b[2]?`${icon('check',{size:12})} کسب شد`:'قفل'}</div></div>`).join('')}</div>
  </div>`;
