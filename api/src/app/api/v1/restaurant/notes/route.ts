...
export const POST = withRestaurantAuth({ rateLimit: 'auth' }, async (req, ctx) => {
  ...
  if (!body) throw Err.validation('متن یادداشت خالی است');
  if (body.length > 2000) throw Err.validation('یادداشت خیلی طولانی است');
  ...
  const note = await db.staffNote.create({
    data: { restaurantId: ctx.restaurant.id, body, pinned: !!b.pinned, authorStaffId: auth.kind === 'staff' ? auth.sub : null, authorName },
  });
...
export const PATCH = withRestaurantAuth({ rateLimit: 'auth' }, async (req, ctx) => {
  const b = await req.json();
  if (!b.id) throw Err.validation('شناسه‌ی یادداشت الزامی است');
...
export const DELETE = withRestaurantAuth({ rateLimit: 'auth' }, async (req, ctx) => {
  ...
  if (!id) throw Err.validation('شناسه‌ی یادداشت الزامی است');
