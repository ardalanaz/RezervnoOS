...
export const GET = withRestaurantAuth({ permission: 'canViewAnalytics' }, async (_req, ctx, params: { userId: string }) => {
  const insight = await db.customerInsight.findUnique({
    where: { restaurantId_userId: { restaurantId: ctx.restaurant.id, userId: params.userId } },
  ...
  const timeline = await db.reservation.findMany({
    where: { restaurantId: ctx.restaurant.id, userId: params.userId },
